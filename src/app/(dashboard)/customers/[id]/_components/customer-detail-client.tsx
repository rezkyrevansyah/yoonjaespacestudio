    ...
    <div className="max-w-4xl space-y-5">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            className="text-red-600 border-red-200 hover:bg-accent hover:border-red-300"
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="p-6">
          <div className="flex items-start gap-5">
            {/* Avatar */}
            <div className="h-16 w-16 rounded-lg bg-accent border-2 border-primary/20 flex items-center justify-center flex-shrink-0">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          ...
          <span className="text-xs text-gray-400 bg-gray-100 px-2 py-1 rounded-full">{totalBookings}</span>
        </div>
        ...
        {sortedBookings.length === 0 ? (
          ...
          <>
            ...
            <div className="hidden sm:block overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    <th className="text-left px-4 py-3 font-medium text-gray-500 text-xs uppercase tracking-wide">Booking ID</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {sortedBookings.map(b => (
                    <tr key={b.id} className="hover:bg-gray-50/60 transition-colors group">
                      ...
                      <td className="px-4 py-2 text-gray-600 text-xs">{formatDate(b.booking_date)}</td>
                      ...
                      <td className="px-4 py-2 text-right font-sans text-xs text-gray-700 font-medium">
                        {formatRupiah(b.total)}
                      </td>
                      <td className="px-4 py-2">
                        <Link
                          href={`/bookings/${b.id}`}
                          className="opacity-0 group-hover:opacity-100 transition-opacity h-6 w-6 flex items-center justify-center rounded text-gray-400 hover:text-primary"
                          title="Lihat Detail"
                        >
            ...
            <div className="sm:hidden divide-y divide-gray-50">
              {sortedBookings.map(b => (
                <Link key={b.id} href={`/bookings/${b.id}`} className="block px-4 py-2 hover:bg-gray-50">
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-500">
                    ...
                    <span className="font-sans font-medium text-gray-700">{formatRupiah(b.total)}</span>
                  </div>
                </Link>
              ))}
            </div>
...
function ContactChip({ icon, value }: { icon: React.ReactNode; value: string }) {
  return (
    <span className="inline-flex items-center gap-2 text-xs bg-gray-100 text-gray-700 px-2.5 py-1 rounded-full">
